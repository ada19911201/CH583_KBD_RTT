 ...
 * @param data 额外数据
 */
void hid_response_success(uint8_t len, void* data)
{
    hid_send_data(len, data);
}
 ...
 * @param offset 
 */
static void get_all_encoders(uint16_t offset)
{
#ifdef MACRO_STORAGE
    response_storage(STORAGE_ENCODER, offset, MAX_HID_PACKET_SIZE);
#else
    hid_response_generic(HID_RESP_UNDEFINED);
#endif
}
 ...
 * @param data 数据
 */
static void set_all_encoders(uint8_t id, uint8_t len, uint8_t* data)
{
#ifdef ENCODER_ENABLED
    set_storage(STORAGE_ENCODER, id * MAX_HID_PACKET_SIZE, data, len);
#else
    hid_response_generic(HID_RESP_UNDEFINED);
#endif
}
...
void hid_on_recv(uint8_t command, uint8_t len, uint8_t* data)
{
    ...
    case HID_CMD_GET_ALL_MACRO:
        ...
        break;
    case HID_CMD_GET_ALL_ENCODER:
        if (len != 2)
            hid_response_generic(HID_RESP_PARAMETER_ERROR);
        else
            get_all_encoders(UINT16_READ(data, 0));
        break;
    ...
    case HID_CMD_SET_ALL_KEYS:
        //data[0]:分包id; 
        set_all_keys(data[0], len, &data[1]);
        break;
    case HID_CMD_SET_ALL_ENCODER:
        //data[0]:分包id; 
        set_all_encoders(data[0], len, &data[1]);
        break;
    case HID_CMD_SET_ALL_FNS:
        set_all_fns(data[0], len - 1, &data[1]);
        break;
    case HID_CMD_SET_SINGLE_CONFIG:
        set_single_config(data[0], len, &data[1]);
        break;
    ...
    case HID_CMD_ENTER_DFU:
        if (len == 1){
            clear_keyboard();
            // send_event_message(FUNCTION_DFU);
        }
        break;
