...
{
    return config_block[3+RGB_LIGHT_CONFIG_P] | (config_block[2+RGB_LIGHT_CONFIG_P] << 8) | (config_block[1+RGB_LIGHT_CONFIG_P] << 16) | (config_block[RGB_LIGHT_CONFIG_P] << 24);
}
/**@brief 写入ws2812存储数据.
 *
 */
...
{
    if ((config_block[3+RGB_LIGHT_CONFIG_P] | (config_block[2+RGB_LIGHT_CONFIG_P] << 8)
            | (config_block[1+RGB_LIGHT_CONFIG_P] << 16) | (config_block[RGB_LIGHT_CONFIG_P] << 24))
        != val) {
        config_block[3] = (val >> 0);//最大256
        config_block[2] = (val >> 8);
        config_block[1] = (val >> 16);
        config_block[0] = (val >> 24);
